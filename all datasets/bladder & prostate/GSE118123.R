...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE118nnn/GSE118123/matrix/"
...
gse <- getGEO(filename = "GSE118123_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',3),rep('CTRL',3)))
...
write.csv2(subtable_result,"GSE118123_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "bp1_GSE118123", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "bp1_GSE118123_correspondence.txt" )
}
