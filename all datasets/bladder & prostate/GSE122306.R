...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/bladder & prostate")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE122nnn/GSE122306/matrix/"
...
gse <- getGEO(filename = "GSE122306_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',6),rep('CTRL',6)))
...
write.csv2(subtable_result,"GSE122306_table(1).csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "bp2_GSE122306", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "bp2_GSE122306_correspondence.txt" )
}
