...
# url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE63nnn/GSE63067/matrix/"
...
# gse <- getGEO(filename = "GSE63067_series_matrix.txt.gz")

# 3. Download the GEO dataset only if not downloaded
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE63nnn/GSE63067/matrix/"
filename <- "GSE63067_series_matrix.txt.gz"
...
d <- factor(c(rep('CD',11),rep('CTRL',7)))
...
write.csv2(subtable_result,"GSE63067_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "LC1_GSE63067", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "LC1_GSE63067_correspondence.txt" )
}
