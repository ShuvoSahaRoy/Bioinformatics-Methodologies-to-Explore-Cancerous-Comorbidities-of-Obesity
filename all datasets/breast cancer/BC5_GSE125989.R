...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE125nnn/GSE125989/matrix/"
...
gse <- getGEO(filename = "GSE125989_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',22),rep('CTRL',10)))
...
write.csv2(subtable_result,"GSE125989_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "BC5_GSE125989", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "BC5_GSE125989_correspondence.txt" )
}
