...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE124nnn/GSE124646/matrix/"
...
gse <- getGEO(filename = "GSE124646_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',90),rep('CTRL',10)))
...
write.csv2(subtable_result,"GSE124646_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "BC4_GSE124646", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "BC4_GSE124646_correspondence.txt" )
}
