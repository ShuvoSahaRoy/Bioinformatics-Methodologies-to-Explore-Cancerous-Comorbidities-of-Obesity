...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/breast cancer")
...
#----------------------------------
# First Example
# A) GSE107300 Alzheimer's Disease (GPL96)

# 3. Download the GEO dataset 
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE107nnn/GSE107300/matrix/"
...
gse <- getGEO(filename = "GSE107300_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('AD',6),rep('CTRL',6)))
...
write.csv(subtable_result,"GSE107300_table.csv")
...
printGraph(AD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "BC2_GSE107300", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "BC2_GSE107300_correspondence.txt" )
}
