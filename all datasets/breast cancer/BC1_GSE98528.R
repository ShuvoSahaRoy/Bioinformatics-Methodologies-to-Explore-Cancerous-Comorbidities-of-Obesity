...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE98nnn/GSE98528/matrix/"
...
gse <- getGEO(filename = "GSE98528_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',39),rep('CTRL',9)))
...
write.csv2(subtable_result,"GSE98528_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "BC1_GSE98528", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "BC1_GSE98528_correspondence.txt" )
}
