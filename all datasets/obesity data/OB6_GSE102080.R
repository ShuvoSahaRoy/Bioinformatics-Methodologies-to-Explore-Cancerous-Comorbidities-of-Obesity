...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE102nnn/GSE102080/matrix/"
...
gse <- getGEO(filename = "GSE102080_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CTRL', 2),rep('OB',2)))
...
write.csv2(subtable_result,"GSE102080_table.csv")
...
printGraph(OB_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "OB6_GSE102080", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "OB6_GSE102080_correspondence.txt" )
}
