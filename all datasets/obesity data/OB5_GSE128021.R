...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/obesity data/ob5")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE128nnn/GSE128021/matrix/"
...
gse <- getGEO(filename = "GSE128021_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CTRL', 3),rep('OB',3)))
...
write.csv2(subtable_result,"GSE128021_table.csv")
...
printGraph(OB_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "OB5_GSE128021", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "OB5_GSE128021_correspondence.txt" )
}
