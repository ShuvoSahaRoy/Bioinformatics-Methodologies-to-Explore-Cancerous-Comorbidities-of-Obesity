...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/obesity data/ob2")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE69nnn/GSE69039/matrix/"
...
gse <- getGEO(filename = "GSE69039_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CTRL', 4),rep('OB',14)))
mod <- model.matrix(~0+d)
fit_1 <- lmFit(gse, mod)
contr <- makeContrasts(dOB-dCTRL, levels = mod)
...
write.csv2(subtable_result,"GSE69039_table.csv")
...
printGraph(OB_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "OB2_GSE69039", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "OB2_GSE69039_correspondence.txt" )
}
