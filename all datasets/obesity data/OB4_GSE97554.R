...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/obesity data/ob4")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE97nnn/GSE97554/matrix/"
...
gse <- getGEO(filename = "GSE97554_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CTRL', 6),rep('OB',5)))
...
write.csv2(subtable_result,"GSE97554_table(1).csv")
...
printGraph(OB_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "OB4_GSE97554", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "OB4_GSE97554_correspondence.txt" )
}
