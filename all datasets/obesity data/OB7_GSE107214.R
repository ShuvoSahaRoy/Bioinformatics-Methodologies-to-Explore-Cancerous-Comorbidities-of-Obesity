...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/obesity data/ob7")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE107nnn/GSE107214/matrix/"
...
gse <- getGEO(filename = "GSE107214_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CTRL', 21),rep('OB',21)))
...
write.csv2(subtable_result,"GSE107214_table.csv")
...
printGraph(OB_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "OB7_GSE107214", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "OB7_GSE107214_correspondence.txt" )
}
