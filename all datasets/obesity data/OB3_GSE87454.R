...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/obesity data/ob3")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE87nnn/GSE87454/matrix/"
...
gse <- getGEO(filename = "GSE87454_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CTRL', 4),rep('OB',28)))
...
write.csv2(subtable_result,"GSE87454_table.csv")
...
printGraph(OB_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "OB3_GSE87454", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "OB3_GSE87454_correspondence.txt" )
}
