...
setwd("C:/Users/SSROY/Desktop/bio/all datasets/kidney")
...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE105nnn/GSE105261/matrix/"
filename <- "GSE105261_series_matrix.txt.gz"
...
# 
# # 4. Convert download dataset in usable class
# gse <- getGEO(filename = "GSE105261_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',35),rep('CTRL',9)))
...
write.csv2(subtable_result,"GSE105261_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "KD1_GSE105261", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "KD1_GSE105261_correspondence.txt" )
}
