...
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE117nnn/GSE117890/matrix/"
filename <- "GSE117890_series_matrix.txt.gz"
...
# 
# # 4. Convert download dataset in usable class
# gse <- getGEO(filename = "GSE117890_series_matrix.txt.gz")

# 5. Create matrix design for limma and calculate differential expression 
d <- factor(c(rep('CD',5),rep('CTRL',6)))
...
write.csv2(subtable_result,"GSE117890_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "KD2_GSE117890", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "KD2_GSE117890_correspondence.txt" )
}
