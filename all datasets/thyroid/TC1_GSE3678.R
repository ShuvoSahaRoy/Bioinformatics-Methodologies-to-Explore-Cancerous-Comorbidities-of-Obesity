...
# url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE3nnn/GSE3678/matrix/"
...
# gse <- getGEO(filename = "GSE3678_series_matrix.txt.gz")

# 3. Download the GEO dataset only if not downloaded
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE3nnn/GSE3678/matrix/"
filename <- "GSE3678_series_matrix.txt.gz"
...
d <- factor(c(rep('CD',7),rep('CTRL',7)))
...
write.csv2(subtable_result,"GSE3678_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "TC1_GSE3678", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "TC1_GSE3678_correspondence.txt" )
}
