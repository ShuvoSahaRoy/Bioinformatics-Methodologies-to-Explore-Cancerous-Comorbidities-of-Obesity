...
# url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE85nnn/GSE85457/matrix/"
...
# gse <- getGEO(filename = "GSE85457_series_matrix.txt.gz")


# 3. Download the GEO dataset only if not downloaded
url <- "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE85nnn/GSE85457/matrix/"
filename <- "GSE85457_series_matrix.txt.gz"
...
d <- factor(c(rep('CD',4),rep('CTRL',3)))
...
write.csv2(subtable_result,"GSE85457_table.csv")
...
printGraph(CD_GOdata,resultFisher,firstSigNodes = 5, fn.prefix = "TC3_GSE85457", useInfo = "all", pdfSW = TRUE)
...
for (i in 1:length(terms))
{
  ...
  cat(term,"genes:",genes_term_2,"\n", append = TRUE, file = "TC3_GSE85457_correspondence.txt" )
}
